from py5 import sqrt, mag, Py5Vector
...
class Gesture(object):
    ...
    def __init__(self, mw, mh):
        self.w = mw
        self.h = mh
        self.path = tuple(Py5Vector() for _ in range(capacity))
        self.polygons = tuple(Polygon() for _ in range(capacity))
    ...
    def add_point(self, x, y):
        if self.n_points >= capacity:
            ...
            self.path[self.n_points].xyz = x, y, p
            self.n_points += 1
            if self.n_points > 1:
                self.exists = True
    ...
    def get_pressure_from_velocity(self, v):
        ...
        min_p = 0.02
        if self.n_points > 0:
            old_p = self.path[self.n_points - 1].z # z for p
        else:
            old_p = 0
    ...
    def set_pressures():
        ...
        for i in range(self.n_points):
            self.path[i].z = sqrt((1.0 - cos(t)) * 0.5)
            t += u
    ...
    def compile(self):
        ...
        for i in range(1, n_path_points):
            ...
            p1x = p1.x
            p1y = p1.y
            radius1 = max(mint, taper * p1.z * self.thickness)
